...
WHISPER_DRAMA_PROMPT = "You are a professional Chinese subtitle transcription engine specialized in TV dramas, movies, and web series. Detect Mandarin Chinese speech accurately, preserve punctuation naturally, split subtitles naturally according to speech pauses, each subtitle containing one complete thought, preserve names exactly as spoken, preserve emotional expressions (唉, 啊, 哼, 呀, 哈哈, 嗯), and output Simplified Chinese."
...
class WhisperWorker(QThread):
    ...
    def __init__(self, audio_path: str, model_size: str = "base", language: str = "zh"):
        super().__init__()
        self.audio_path = audio_path
        self.model_size = model_size
        self.language = language if language != "Auto" else "zh"

    def run(self):
        try:
            m_str = self.model_size.lower()
            if "large" in m_str or "qwen" in m_str or "chirp" in m_str or "soniox" in m_str:
            ...
            self.progress.emit(10, f"Loading Chinese STT Engine ({self.model_size})...")
            ...
            try:
                ...
                self.progress.emit(30, f"Transcribing Chinese audio with {self.model_size}...")
                segments, info = model.transcribe(
                    self.audio_path,
                    language=self.language,
                    initial_prompt=WHISPER_DRAMA_PROMPT,
                    beam_size=5,
                    vad_filter=True
                )
